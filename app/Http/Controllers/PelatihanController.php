...
class PelatihanController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->back()->with('galat','gagal menambahkan pelatihan baru')->withErrors(['error' => 'Terjadi kesalahan saat menambahkan data'])->withInput();
        }         
    }
    ...
    public function show($slug)
    {
        ...
        if ($row->isEmpty()) {
            return redirect()->route('admin.course')->with('rusak', 'Pelatihan tidak ditemukan');
        }
        $id = $row[0]['id'];
...
{        
    ...
    try {
        Pelatihan::where('slug', $slug)->update($validatedData);
        return redirect(route('admin.course'))->with('warning','pelatihan : '.$pelatihan->title.' telah berhasil diubah');
    }catch (\Exception $e){
        return redirect()->back()->with('galat','gagal mengubah data pelatihan')->withErrors(['error' => 'Terjadi kesalahan saat menambahkan data'])->withInput();
    }       
}
    ...
    public function destroy($slug)
    {
        ...
        Peserta::where('pelatihan_id', $pelatihan['id'])->delete();

        // Hapus record pelatihan
        $pelatihan->delete();
        return redirect(route('admin.course'))->with('danger','Pelatihan: '.$pelatihan->title.' telah berhasil dihapus');
    }
    ...
    public function exportParticipants($pelatihanId)
    {
        // Ambil judul pelatihan dari database
        $pelatihan = Pelatihan::findOrFail($pelatihanId);
        $judulPelatihan = $pelatihan->title;

        // Ambil tanggal saat ini dalam format 'Y-m-d' (misalnya: 2024-07-26)
        $tanggalSekarang = now()->format('Y-m-d');

        // Buat nama file yang diinginkan
        $namaFile = "{$tanggalSekarang}_peserta_{$judulPelatihan}.xlsx";

        // Ekspor file Excel dengan nama yang dinamis
        return Excel::download(new ParticipantsExport($pelatihanId), $namaFile);
    }
}
